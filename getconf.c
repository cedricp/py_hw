  ...
  if (spec != NULL)
    {
      ...
      if (i == nspecs)
        error (2, 0, ("unknown specification \"%s\""), spec);
...
# endif
            {
              ...
              error (4, errno, ("Couldn't execute %s"), getconf_name);
            }
          default:
            break;
        }
  ...
  error (2, 0, ("Unrecognized variable `%s'"), argv[ai]);
  /* NOTREACHED */
  return 2;
}
